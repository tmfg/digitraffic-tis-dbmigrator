CREATE TABLE ticket
(
    id        BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (START WITH 1000000 INCREMENT BY 1),
    public_id TEXT DEFAULT nanoid(),
    "format"  TEXT NOT NULL,
    url       TEXT NOT NULL,
    etag      TEXT
);

CREATE TABLE step
(
    id        BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (START WITH 1100000 INCREMENT BY 1),
    ticket_id BIGINT       NOT NULL,
    phase     TEXT         NOT NULL,
    started   TIMESTAMP(3) NOT NULL,
    CONSTRAINT fk_step_ticket_ticket_id FOREIGN KEY (ticket_id) REFERENCES ticket (id) ON DELETE CASCADE
);

CREATE TABLE validation
(
    id        BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (START WITH 1200000 INCREMENT BY 1),
    ticket_id BIGINT NOT NULL,
    CONSTRAINT fk_validation_ticket_ticket_id FOREIGN KEY (ticket_id) REFERENCES ticket (id) ON DELETE CASCADE
);

CREATE TABLE conversion
(
    id        BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (START WITH 1300000 INCREMENT BY 1),
    ticket_id BIGINT NOT NULL,
    CONSTRAINT fk_conversion_ticket_ticket_id FOREIGN KEY (ticket_id) REFERENCES ticket (id) ON DELETE CASCADE
);

CREATE TABLE resource
(
    id        BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (START WITH 1400000 INCREMENT BY 1),
    ticket_id BIGINT NOT NULL,
    path      TEXT   NOT NULL,
    CONSTRAINT fk_resource_ticket_ticket_id FOREIGN KEY (ticket_id) REFERENCES ticket (id) ON DELETE CASCADE
);


