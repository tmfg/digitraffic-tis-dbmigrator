...
(
    ...
    updated   TIMESTAMP(3),
    completed TIMESTAMP(3),
    CONSTRAINT fk_task_entry_entry_id FOREIGN KEY (entry_id) REFERENCES entry (id) ON DELETE CASCADE,
    UNIQUE (entry_id, name)
);

CREATE INDEX idx_task_name ON task
    (
     name
        );
...
(
    ...
    name     TEXT   NOT NULL,
    path     TEXT   NOT NULL,
    CONSTRAINT fk_package_entry_entry_id FOREIGN KEY (entry_id) REFERENCES entry (id) ON DELETE CASCADE
);
...
(
    ...
    CONSTRAINT fk_error_task_task_id FOREIGN KEY (task_id) REFERENCES task (id) ON DELETE CASCADE,
